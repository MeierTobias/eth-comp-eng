    ...
    \item \texttt{MemRead}/\texttt{MemWrite} are \textbf{both} needed as some instructions don't use data memory at all.
          \begin{itemize}
              ...
              \item Use 2 wires. If both =0: Turn power off when it is not required.
                    ...
                    % TODO: For sw the memory block only gets one address, if MemRead and MemWrite are both 1, then the memory block will both write and read from the same address -> undefined behavior
