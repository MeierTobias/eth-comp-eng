...
\texttt{JAL} stores the address of the instruction following the jump (PC+4) into the register \texttt{rd}. The standard software calling convention uses \texttt{x1/ra} as the return address register.

The registers that are used after the PC returned to the line below the jump instruction have the be saved to the stack. The table~\ref{registers} shows all the registers that have to be saved by the \textbf{caller}.
...
sw ra, 0(sp)
# jump to the function located at a0
jalr ra, a0
# After a0 is finished it resumes here and loads the return address from the stack 
lw ra, 0(sp)
