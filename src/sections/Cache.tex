...
\textbf{Remarks}
\begin{itemize}
    \item In contrast to TLB cache, data/instruction cache is accessed with \textbf{physical} addresses to avoid flushing the cache when switching vmas. % TODO: Is this correct?
    \item In the case of TLB, the data remain in DRAM but the address translation is saved in cache. For data/instruction cache the actual data are cached using their physical addresses. % TODO: Duplicated explanation. TLB stores addresses and data cache data.
\end{itemize}
...
% TODO: Should we remove this section and move the information into the corresponding subsections? 
...
\begin{itemize}
    \item \textbf{Offset} within the cache line (6 bits: \fncode{cccccc})
    \item \textbf{Slot} (for 4 sets we need 2 bits: \fncode{ss})
    \item \textbf{Tag} (24 bits: \fncode{tt..tttt})
\end{itemize}
