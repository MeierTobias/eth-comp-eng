...
In set-associative cache, a DRAM entry can go to any slot in their set. The \textbf{wayness} is the number of entries per set (e.g. a 4-way 4-set associative has 4 sets with 4 slots in each one of them).

\ptitle{Wayness-Set Tradeoff}
\begin{itemize}
    \item In a \textbf{direct-mapped} (1-way n-set associative) cache, one has $\mathcal{O}(1)$ lookup but the chance for eviction is higher as there is no way to dodge a conflict because only 1 cache slot is assigned per DRAM block.
    \item In a \textbf{fully-associative} (n-way 1-set associative) cache one has $\mathcal{O}(n)$ lookup but the chance for eviction is small as one can dodge conflicts.
