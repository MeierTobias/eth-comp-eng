...
\newpar{}
As mentioned in Table~\ref{tab:opcode}, setting the four bits (\code{Branch, RegWrite, MemWrite, MemRead}) to zero creates and pushes a \code{NOP} instruction into the pipeline, resulting in a temporary pause of the CPU. To achieve this, the hazard detection unit selects the zero input of the multiplexer yielding \textbf{zero control registers}. Note that the PC can be manipulated.
\begin{center}
