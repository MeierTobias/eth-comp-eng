...
%TODO: I find this example confusing: it suggests that and/or have to be managed differently even though they are the same and only their position is relevant.
% Additionally, I don't see it adding value if one understands the previous subsubsection

% \begin{examplesection}[Example]\label{example_hazard_detection}
%     Even though the \code{and} and the \code{or} instruction face a similar hazard, the comparison required to detect the hazard is specific to their relative location w.r.t.\ the \code{sub} instruction.
    
%     \includegraphics[width=\linewidth]{example_hazard_detection.png}
% \end{examplesection}
...
% By passing the result of the execution stage directly into the input of the next execution stage the \code{add}\textleftarrow\code{sub} RAW hazard can be avoided.
...
\end{center}
The comparison must either happen with the \code{EX/MEM} or with the \code{MEM/WB} stage depending on the instruction order.
...
\newpar{}
As mentioned in Table~\ref{tab:opcode}, setting the four bits (\code{Branch, RegWrite, MemWrite, MemRead}) to zero creates and pushes a \code{NOP} instruction into the pipeline, resulting in a temporary pause of the CPU. 
% \begin{itemize}
...
\end{center}
% Note that the comparison already happens at the \code{ID/EX} stage and that the PC is mainpulated! % TODO: don't all copmarisons happen at ID/EX stage?
